...
namespace eShopSolution.ViewModels.ProductModels.Validators
{
    public class ManageProductCreateValidator : AbstractValidator<ProductCreateRequest>
    {
        ...
        {
            RuleFor(x => x.Price).LessThanOrEqualTo(28).ScalePrecision(2, 2).WithMessage("Weight cannot be greater than 28 and must be a precison of 2");
            RuleFor(x => x.OriginalPrice).LessThanOrEqualTo(28).ScalePrecision(2, 2).WithMessage("Weight cannot be greater than 28 and must be a precison of 2");
